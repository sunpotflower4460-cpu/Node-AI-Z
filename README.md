...
結晶思考モデルを観察・体験・自己修正していくための実験アプリです。Node / Home / Revision / Memory を通じて、育つ知性の構造を見ていきます。現在は **SRM-3 / Memory Promotion** の段階で、段階昇格つきの memory と plasticity 再構築が入っています。
...
返答ごとに revision entry と変更候補を生成し、memory に蓄積します。proposed change 単位で ephemeral / provisional / promoted / reverted を再計算し、keep / soften / revert / lock を通じて段階昇格と plasticity 反映を制御します。
...
- **SRM-3 昇格ロジック**（keep 蓄積 / 再登場 / soften / revert / lock を加味して memory status を自動更新）
- **status 重みつき plasticity 再構築**（ephemeral / provisional / promoted / reverted の差を node / relation / pattern / home trigger / tone bias に反映）
- **Observe の Promotion / Memory 可視化**（Recently Promoted / Provisional Queue / Revision Growth Summary / Top Thickened Pipes）

## 今後の予定

- SRM-4（Memory summarization / semantic clustering）の実装
- plasticity の反映方針の微調整と memory 昇格条件の運用改善
- tuning UI のさらなる強化
- より自然な体験会話と対話継続性の向上
