...
結晶思考モデルを観察・体験・自己修正していくための実験アプリです。Node / Home / Revision / Memory を通じて、育つ知性の構造を見ていきます。現在は **SRM-3（Memory 昇格ロジック + Surface API 接続 v1）** 段階です。
...
### Base API Selection / Surface API v1
Node-AI-Z では基準APIを選べます。内部の Node / Home / Revision / Memory は共通のまま、**Experience Mode の最終返答だけ**を Surface API で自然化します。API key がない場合や route / provider エラー時は `internal_mock` に安全フォールバックします。
...
- **観察研究モード / 体験モードの切り替え**
- 体験モードの二段構成（`runNodePipeline` + `buildStudioViewModel` で内部状態を作り、Surface API で最終返答だけ自然化）
- 体験モード会話のセッション保持と localStorage 保存
- 体験モードから観察研究モードへ戻る導線
- **基準API選択 v1（`internal_mock` / `openai` / `anthropic` / `google`、利用可否は server route で判定）**
...
Vercel では **Environment Variables** に次を設定してください。
...
- API key は **フロントに直書きしません**
- key は `/api/providers` と `/api/surface-chat` の **サーバー側 route だけ**で読みます
- API key が未設定の provider は UI 上で unavailable になり、Experience Mode では自動で `internal_mock` にフォールバックします
- route error / timeout / provider 4xx / 5xx の場合も `internal_mock` に戻ります
...
`test:run` では promotion rules / promoteRevisionState / surface prompt / provider fallback を含む 80 テストが通ることを確認済みです（詳細: `docs/reports/`）。
