...
- **ISR v2.2（Integrated Signal Runtime v2.2）実装済み**: Meaning Chunk → Feature Activation → Temporal Decay → Refractory Gating → Feature Inhibition → Dynamic Threshold → Recurrent Self Loop → Lateral Inhibition → Node Activation の完全パイプライン。発火は1回で確定せず、数ステップの収束ループを経る。抑制性シグナルと動的閾値により、反応の前景化が少し脳寄りになった。predictive coding / hierarchical proto meaning / somatic marker は次段階。
...
| ファイル | 役割 |
|---|---|
| `src/signal/temporalTypes.ts` | `TemporalFeatureState` / `RecurrentLoopResult<T>` 型定義 |
| `src/signal/applyTemporalDecay.ts` | 指数減衰を feature 強度に適用 |
| `src/signal/applyRefractoryGating.ts` | 不応期による再発火抑制 |
| `src/signal/runRecurrentSelfLoop.ts` | 収束条件付き再帰 self/belief ループ |
| `src/signal/applyLateralInhibition.ts` | winner-take-more 側方抑制 |
