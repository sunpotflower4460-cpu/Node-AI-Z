...
結晶思考モデルを観察・体験・自己修正していくための実験アプリです。Node / Home / Revision / Memory を通じて、育つ知性の構造を見ていきます。現在は **SRM-2 / Provisional Plasticity** の段階で、SRM-3（Memory 昇格ロジック）はまだ入っていません。
...
- **暫定 plasticity 反映**（node / relation / pattern / home trigger / tone bias の小さな補正を可視化し、次回 run に反映）

## 今後の予定

- SRM-3（Memory 昇格ロジック）の実装
