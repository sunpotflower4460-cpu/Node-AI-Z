...
- **観察モード** — 内部で何が起きているかを見る場所（発火・成長・リスク・pipeline・trace・dashboard）
- **体験モード** — 自然に話して反応を受け取る場所（会話・返答・裏側へのリンク）

観察モードは研究室。体験モードは会話室。

体験モードでの返答後、「裏側を見る」リンクから観察モードの該当タブ（発火・成長・リスク）に直接移動できます。

脳エンジン（New Signal / Legacy / LLM）は「体験モード」とは独立した概念です。体験モードは画面モードであり、どの脳エンジンを裏で使うかとは別に扱われます。

Simple View では会話体験が中心に表示されます。Research View では使用エンジン・Surface Provider・Runtime 情報が確認できます。

## UI Clarity Pass v6-3

Analyze フローが 3 つの状態に明確化されました:

- **before（入力待ち）** — まだ観察結果がない / 入力待ち
- **analyzing（実行中）** — 内部パイプラインが動いている / ステップが見える
- **completed（結果あり）** — 今回何が起きたか / 次に見るべき場所が分かる

Analyze 後、UI は何が変わったかを要約し、次に見るべきタブ（発火・成長・先生・リスク・概要）を案内します。

assembly が増えない場合も「baseline が作られた」として意味づけされ、失敗として見せません。
...
- `runLlmModeRuntime` — じぶん会議方式の runtime。API provider を使用して応答を生成
...
* **llm_mode**: personal_branch のみ read、write なし
...
- **Active Attention**: 全候補を均等に見るのではなく、unstable but repeating assembly / teacher dependency が高い bridge / recall failed bridge / promotion readiness が高い候補 / sequence mismatch / contrast unclear を注意 budget 内で選び、次ターンの観察方針を自分で立てる。
- **Contrast Learning**: assembly 間の overlap / recurrence / 時間差から same object / similar but different / same category / different category / unrelated / unknown を分け、何でも同じに束ねない。
- **Sequence Memory**: recent assembly の流れを sequence memory に蓄積し、次に来そうな assembly を予測しつつ、prediction mismatch も記録する。
- **Multi-timescale Plasticity**: short / mid / long-term weight を別々に持ち、今の反復・休止 replay・時間経過の影響を分離して扱う。
- **Offline Dream Exploration**: rest/consolidation 中に weak hypothesis bridge を試し、contrast / sequence / plasticity support があるものだけを tentative self bridge として残す。
- **Internal Question**: 曖昧さや mismatch をそのまま流さず、小さな問いとして内部化し、priority 順に次の replay / compare / ask teacher / observe を決める。
- **Action / Outcome Learning (v7)**: Active Attention が選んだ候補に対して、replay / bridge test / ask teacher / contrast compare / prediction / suppression / hold を小さな内的行動として実行し、その結果を reward / error / outcome memory に記録する。
- **Global Modulators / Reconsolidation / Development**: novelty / uncertainty / fatigue / stability / overload を全体状態として更新し、prediction surprise や失敗した記憶を reconsolidation で再編集しつつ、Stage 1〜8 の発達段階で internal action と export 候補を段階的に解放する。

これにより、Signal Mode は「発火して育つ」だけでなく、「休んで整理し、注意資源を持ち、抑制し、差分と順序を学び、複数タイムスケールで重みを保ち、夢のように弱い仮説を試し、自分で問いを立てる」状態になります。
