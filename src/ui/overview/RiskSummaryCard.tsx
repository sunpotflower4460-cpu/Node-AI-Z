...
export const RiskSummaryCard = ({ risk, detailMode }: RiskSummaryCardProps) => {
  const metrics = [
    { simpleLabel: '過結合傾向', researchLabel: 'Overbinding Risk', value: risk.overbindingRisk },
    { simpleLabel: '誤結合傾向', researchLabel: 'False Binding Risk', value: risk.falseBindingRisk },
    { simpleLabel: '先生過信傾向', researchLabel: 'Teacher Overtrust Risk', value: risk.teacherOvertrustRisk },
    { simpleLabel: '夢ノイズ傾向', researchLabel: 'Dream Noise Risk', value: risk.dreamNoiseRisk },
  ]

  const sectionLabel = detailMode === 'research' ? 'Risk Summary' : 'リスク'
  ...
  return (
    <section className="rounded-[28px] border border-slate-800 bg-slate-950/90 p-5 shadow-[0_18px_50px_-30px_rgba(244,63,94,0.28)]">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {metrics.map((metric) => (
          <div key={metric.researchLabel} className="rounded-2xl border border-slate-800 bg-slate-900/80 p-4">
            <p className="text-xs font-semibold text-slate-300">{detailMode === 'research' ? metric.researchLabel : metric.simpleLabel}</p>
