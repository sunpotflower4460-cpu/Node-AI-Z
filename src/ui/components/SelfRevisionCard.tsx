    ...
    <div className="bg-white rounded-2xl shadow-sm border border-indigo-200 p-5 space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1">
          <h3 className="text-xs font-bold uppercase tracking-widest text-indigo-600 mb-1.5 flex items-center gap-1.5">
            <RefreshCcw className="w-3.5 h-3.5" /> Self-Revision
          </h3>
          <p className="text-sm text-slate-700 font-medium leading-relaxed">{entry.note}</p>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div className="text-right">
            <div className="text-xs font-bold uppercase tracking-wide text-slate-500">{entryStatusMeta.label}</div>
      ...
      {hasIssues && (
        <div className="bg-amber-50/50 border border-amber-200 rounded-xl p-3">
          <h4 className="text-[11px] font-bold uppercase tracking-widest text-amber-700 mb-2">Issue Tags</h4>
          <div className="flex flex-wrap gap-1.5">
            {entry.issueTags.map((tag, index) => (
              <span key={index} className="px-2 py-1 rounded-lg bg-amber-100 text-amber-800 text-xs font-semibold border border-amber-200">
                {tag}
              </span>
            ))}
          </div>
      ...
      {hasChanges && (
        <div className="space-y-2">
          <h4 className="text-xs font-bold uppercase tracking-widest text-slate-600">Proposed Changes ({entry.proposedChanges.length})</h4>
          {entry.proposedChanges.map((change) => {
            ...
            return (
              <div key={change.id} className={`rounded-xl border p-3 ${getRevisionStatusMeta(change.status).panelClass}`}>
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="flex-1">
                    <div className="mb-1.5 flex flex-wrap items-center gap-2">
                      <span className="text-[11px] font-bold uppercase tracking-wide px-2 py-0.5 bg-white/70 rounded-md">
                        {getRevisionKindLabel(change.kind)}
                      </span>
                      <span className="text-xs font-mono text-slate-600">{change.key}</span>
                      <span className={`rounded-full border px-2 py-0.5 text-[11px] font-bold ${getRevisionStatusMeta(change.status).badgeClass}`}>
                        {getRevisionStatusMeta(change.status).label}
                      </span>
                      {isLocked && (
                        <span className="rounded-full border border-indigo-300 bg-indigo-100 px-2 py-0.5 text-[11px] font-bold text-indigo-700 flex items-center gap-1">
                ...
                {onTuningAction && !isLocked && (
                  <div className="flex gap-1.5 mt-2 pt-2 border-t border-current/20">
                    {!isReverted && (
                      <>
                        <button
                          ...
                          className="flex-1 px-2 py-1.5 text-[11px] font-bold rounded-lg bg-green-100 text-green-700 hover:bg-green-200 transition-colors"
                        ...
                        <button
                          ...
                          className="flex-1 px-2 py-1.5 text-[11px] font-bold rounded-lg bg-yellow-100 text-yellow-700 hover:bg-yellow-200 transition-colors"
                    ...
                    <button
                      ...
                      className={`flex-1 px-2 py-1.5 text-[11px] font-bold rounded-lg transition-colors ${isReverted ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-red-100 text-red-700 hover:bg-red-200'}`}
                    ...
                    {!isReverted && (
                      <button
                        ...
                        className="px-2 py-1.5 text-[11px] font-bold rounded-lg bg-indigo-100 text-indigo-700 hover:bg-indigo-200 transition-colors flex items-center gap-1"
                ...
                {isLocked && (
                  <div className="mt-2 pt-2 border-t border-current/20 flex items-center gap-1.5">
                    ...
                    <span className="text-[11px] font-bold tracking-wider opacity-75">
